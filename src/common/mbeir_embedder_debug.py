    ...
    for split_name, split_dir, dataset_name_list, cand_pool_name_list in splits:
        for dataset_name, cand_pool_name in zip(dataset_name_list, cand_pool_name_list):
            ...
            embedding_list, id_list = generate_embeds_and_ids_for_dataset_with_gather(
                model, data_loader, device=config.dist_config.gpu_id, use_fp16=config.embed_config.use_fp16
            )
...
def main(config):
    ...
    # Generate embeddings
    generate_embeds_for_config(model=model, img_preprocess_fn=img_preprocess_fn, tokenizer=tokenizer, config=config)
...
if __name__ == "__main__":
    ...
    # Set dataset config
    config.embed_config.cand_pools_config.cand_pools_name_to_embed = ['val_%d_%d' %(args.data_num, args.val_size)]
    config.embed_config.val_datasets_config.correspond_cand_pools_name = ['val_%d_%d' %(args.data_num, args.val_size)]
    config.embed_config.val_datasets_config.datasets_name = ['arxiv_%d_%d'%(args.data_num, args.val_size)]
    
    config.model.ckpt_config.ckpt_dir = args.ckpt_dir
